...
const validateProveedorData = (data) => {
  const { nombre, apellido, sector, telefono, compania } = data;
  const errors = [];
...
router.post('/add', isLoggedIn, async (req, res) => {
    const { nombre, apellido, sector, telefono, compania } = req.body;
    ...
    try {
      ...
      const proveedorData = { Nombre: nombre, Apellido: apellido, Sector: sector };
      ...
      const telefonoData = { Numero: telefono, Compania: compania, Cod_Proveedor: nuevoCodProveedor, Cod_Empleado: null };
      ...
      res.json({ success: true, data: proveedores }); // Retornar los proveedores actualizados
    } catch (error) {
