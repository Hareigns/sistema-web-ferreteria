...
router.post("/api/ventas", isLoggedIn, async (req, res) => {
  ...
  if (!codigo_venta || !codigo_empleado || !total_venta || !detalles_venta) {
  ...
  try {
    ...
    await connection.query(
      `INSERT INTO Venta (Cod_Venta, Fecha_Venta, Cod_Cliente, Total_Venta)
       VALUES (?, ?, ?, ?)`,
      [codigo_venta, fechaVentaValida, codigo_empleado, total_venta]
    );
...
router.post("/api/ventas", isLoggedIn, async (req, res) => {
  ...
  // Validaciones básicas
  if (!codigo_empleado || !total_venta || !detalles_venta) {
    return res.status(400).json({ 
      success: false, 
      message: "Datos incompletos" 
    });
  ...
  try {
    ...
    const [resultVenta] = await connection.query(
      `INSERT INTO Venta (Fecha_Venta, Cod_Cliente, Total_Venta)
       VALUES (?, ?, ?)`,
      [fechaVentaValida, codigo_empleado, total_venta]
    );
